...
const SaveButton = ({ onClick, isLoading, ...props }: ISaveButton) => {
  return (
    <button
      onClick={onClick}
      className='group relative overflow-hidden rounded-3xl border-[1px] border-green-300 bg-stone-100 px-5 py-[6px] font-semibold text-gray-800 transition-all duration-1000'
      {...props}
    >
      <p className='relative z-10 flex items-center justify-center gap-2 transition-colors duration-500 group-hover:text-white'>
        <span>
          {isLoading ? (
